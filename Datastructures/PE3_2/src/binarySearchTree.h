...
public:
	...
	//Recursive function to delete all leaf nodes from the tree.
	//You cannot invoke any exiting member functions, except the ones created by you.
	//Postcondition: all leaf nodes are removed from the tree.
	void deleteRecAllLeaves();
...
void bSearchTreeType<elemType>::insert(const elemType& insertItem)
{
    ...
    else
    {
        ...
        while (current != NULL)
        {
            ...
            {
                cerr << "The insert item is already in the list-duplicates are not allowed.";
                cerr << insertItem << "\n";
                return;
            }
...
template<class elemType>
inline void bSearchTreeType<elemType>::deleteRecAllLeaves() {

	int treeLeavesCount=this->treeLeavesCount();
	if(treeLeavesCount==0){
		cout<<"\n Binary tree has no leaves"<<endl;
	}
	else{
...
template <class elemType>
void bSearchTreeType<elemType>::deleteAllLeaves()
{
	// PLEASE IMPLEMENT THE RECURSIVE FUNCTION
	// NOTE: cannot invoke any exiting member functions, except the ones created by you
	this->deleteRecAllLeaves();
