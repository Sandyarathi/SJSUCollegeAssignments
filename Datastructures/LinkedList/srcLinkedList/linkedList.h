...
public:
    ...
    //Postcondition : The smallest info is found and deleted from list.
    virtual void deleteAll(const Type& deleteItem)=0;
    //function to delete all the occurences of the given item from the list
    //PostCondition : All occurences of the item is deleted from the list.
...
inline void linkedListType<Type>::split2(linkedListType<Type>& sublist) {
	...
	if (first == NULL)
	{
		...
		else
		{
			...
			sublist.count = this->count - count;
			this->count = count;
		}
